...
def plot_cpuprof(cpuprof, output, interval = 1, terminaltype = "png"):
    ...
    gd = []
    gd.append(Gnuplot.Data(xvals, piledatas[0], with_ = "filledcurve x1", title = keys[0]))
    for i in range(1, len(keys)):
        gd.append(Gnuplot.Data(xvals, piledatas[i], piledatas[i - 1],
...
if __name__ == "__main__":
    ...
    from profileutils import import_mpstatfile
    cpustatdict = import_mpstatfile(cpufile)

    fprefix = cpufile.rsplit('.', 1)[0]
    output = "{0}core{1}.{2}".format(fprefix, core, terminaltype)
    plot_cpuprof(cpustatdict[core], output, 1, terminaltype)
